...
exports.BtcSimulator = void 0;
class BtcSimulator {
    historicalPrices = [];
    historicalVolatility = 0.0001; // Default per-second vol
    lastUpdateTime = 0;
    constructor() {
        ...
        if (this.historicalPrices.length > 20) {
            ...
            for (let i = 1; i < this.historicalPrices.length; i++) {
                if (this.historicalPrices[i - 1] > 0) {
                    returns.push(Math.log(this.historicalPrices[i] / this.historicalPrices[i - 1]));
                }
            }
     ...
     */
    simulatePricePath(currentPrice, horizonSeconds = 300, // 5 minutes default
    ...
    seed) {
        ...
        const realizedVol = Math.sqrt(pathReturns.reduce((a, b) => a + (b - avgReturn) ** 2, 0) / pathReturns.length);
        return {
            prices,
            timestamps,
            finalPrice: price,
     ...
     */
    generateOrderBook(midPrice, btcVolatility = 0.001, seed) {
     ...
     */
    runBatchSimulations(currentPrice, horizonSeconds, count = 10000, stepMs = 5000) {
        const finalPrices = [];
        let totalJumps = 0;
        for (let i = 0; i < count; i++) {
            const path = this.simulatePricePath(currentPrice, horizonSeconds, stepMs, i * 7919 + Date.now() // Unique seed per simulation
            );
        ...
        const variance = finalPrices.reduce((a, b) => a + (b - avgFinalPrice) ** 2, 0) / count;
...
exports.BtcSimulator = BtcSimulator;
