...
class OTCDataEngine {
    ...
    sampleOTCSize() {
        const r = Math.random();
        if (r < 0.4)
            return 1 + Math.random() * 9; // 1-10 BTC (40%)
        if (r < 0.7)
            return 10 + Math.random() * 40; // 10-50 BTC (30%)
        if (r < 0.9)
            return 50 + Math.random() * 150; // 50-200 BTC (20%)
     ...
     */
    getSnapshot(btcPrice, btcChange5m) {
        const now = Date.now();
     ...
     */
    predictNextFlow(btcPrice, btcChange5m) {
...
exports.OTCDataEngine = OTCDataEngine;
