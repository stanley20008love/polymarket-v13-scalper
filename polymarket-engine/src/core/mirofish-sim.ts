...
export class MiroFishEngine {
  ...
  private agentTemplates: SimulationAgent[] = [];
  ...
  async simulate(
    ...
    klinesClose: number[],
    loopCount: number = 10000
  ): Promise<SimulationResult> {
    const startTime = Date.now();

    logger.info('MiroFish simulation starting', {
    ...
    const context = this.buildMarketContext(
      btcPrice, btcChange5m, btcVolume5m, depthImbalance, klinesClose, orderBook, market
    );
    ...
    const pnlStd = Math.sqrt(
      pnlDistribution.reduce((a, b) => a + (b - avgPnl) ** 2, 0) / loopCount
    );
    ...
    logger.info('MiroFish simulation complete', {
      direction,
   ...
   */
  private runSingleLoop(context: SimulationState, seed: number): {
    direction: 'UP' | 'DOWN';
    expectedPnl: number;
    pricePath: number[];
  } {
  ...
  private boxMullerRandom(rng: () => number): number {
  ...
  recordOutcome(market: string, predictedDirection: 'UP' | 'DOWN', actualOutcome: 'UP' | 'DOWN'): void {
    const entry = this.simulationHistory.find(
      h => h.market === market && !h.actualOutcome
    );
    if (entry) {
